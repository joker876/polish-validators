...
import { BankNameTree } from './iban-bank-name-tree';
import { BANK_NAME_LIST } from './iban-bank-names';
import { FULL_BANK_NAME_LIST } from './iban-bank-names-full';
...
export function isIbanValid(iban: string): boolean {
  ...
  if (countryCode === 'PL' && !BankNameTree.existsCode(bankNameNumber)) {
    return false;
  }
...
export function getBankNameFromIban(iban: string): string | null {
  ...
  const bankNameIndex = BankNameTree.getIndex(bankCode);

  if (!bankNameIndex) {
    return null;
  }
  return BANK_NAME_LIST[bankNameIndex] ?? null;
}
...
export function getFullBankNameFromIban(iban: string): string | null {
  ...
  return FULL_BANK_NAME_LIST[bankNameIndex] ?? null;
}
